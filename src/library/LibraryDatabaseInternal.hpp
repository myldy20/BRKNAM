...
[[nodiscard]] std::string default_root_name(
    const std::filesystem::path& path);
[[nodiscard]] std::string display_name_for(
...
[[nodiscard]] std::optional<double> optional_double(
    sqlite3_stmt* statement, int column);
[[nodiscard]] std::optional<int> optional_int(
    sqlite3_stmt* statement, int column);
...
struct LibraryDatabase::Impl {
  ...
  void migrate();
  [[nodiscard]] std::vector<std::string> tags_for(
      std::int64_t asset_id) const;
